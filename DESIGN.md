...
**Exception — Chassis Identity + Generation card pair on the analyze page:** This specific card pair is intentionally forced to equal height by design decision. The generation card drives the height; the chassis identity card auto-spaces to match using `h-full` with a `flex flex-col` root. Within the chassis card, extra vertical space is distributed proportionally across two regions — the identity fields grid (`flex-auto` + `align-content: space-between`) and the Factory Specs section (`flex-auto` wrapper with `justify-between` on its inner dl) — so both sections breathe rather than one void forming between them. Variant production volume is shown below the VIN header (not in the generation card) so the chassis card has enough content to make the distributed spacing visually reasonable. The "no forced equal-height" principle still applies everywhere else.
